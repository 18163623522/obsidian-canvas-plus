...
var CODE_KEYWORDS = /\b(const|let|var|int|float|double|char|void|bool|return|if|else|for|while|do|switch|case|break|continue|function|class|struct|enum|import|from|def|print|echo|public|private|protected|static|new|this|null|fn|func|package|namespace|using|std|cout|cin|printf|scanf|include|SELECT|INSERT|UPDATE|DELETE|CREATE|local)\b/;
var PROSE_WORDS = /\b(the|an|is|are|was|were|with|very|normal|sentence|sign|inside|and|or|of|to|in|on|we|you|they|this|that|these|those|there|here|what|when|where|which|who|how|why|please|thanks|hello|world)\b/i;
var MATH_CHARSET = /^[A-Za-z0-9\s+\-*/=^_{}().,%!<>|;:'"°²³√π∑∫∞≤≥≠±×÷·∂∇\[\]]+$/;
function detectFormula(text) {
  ...
  if (PROSE_WORDS.test(t)) return false;
  if (!MATH_CHARSET.test(t)) return false;
  if (!/[=^_]/.test(t) || !/[A-Za-z]/.test(t)) return false;
  if (/[;{}]|=>|==|&&|\|\||\/\/|#/.test(t)) return false;
  const longWords = t.match(/[A-Za-z]{3,}/g) ?? [];
  const mathFns = /* @__PURE__ */ new Set(["sin", "cos", "tan", "sqrt", "log", "ln", "exp", "abs", "min", "max", "det", "arg", "deg", "lim"]);
...
var IGNORE_LANGS = /* @__PURE__ */ new Set(["markdown", "plaintext", "ini", "properties"]);
var LANG_SUBSET = [
  "c",
  "cpp",
  "csharp",
...
var CODE_SIGNAL = /([;{}]\s*$)|(\b(?:int|float|double|char|void|bool|long|short|unsigned)\s+\w+\s*[=;(])|(^\s*(#include|import\s+[\w"']|from\s+\w+\s+import|def\s+\w+\s*\(|class\s+\w+|function\s*\w*\s*\(|const\s+\w+\s*=|let\s+\w+\s*=|var\s+\w+\s*=|public\s|private\s|fn\s+\w+|func\s+\w+|package\s+\w+|using\s+\w+|SELECT\s|INSERT\s|CREATE\s|local\s+\w+\s*=))|=>|^\s*\/\//m;
function detectLangByHint(text) {
  const t = text;
...
function detectCode(text) {
  ...
  let result;
  try {
    result = common_default.highlightAuto(t, LANG_SUBSET);
  } catch {
    return null;
  }
